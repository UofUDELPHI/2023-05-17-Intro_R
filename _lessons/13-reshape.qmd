...
The "wide"-labelled format in the example below is actually tidy:
...
will contain the variable names (`variable`) and `values_to` for naming the column that will contain the values of each variable. We supply these new column
names as strings.



Typically, long-format is useful for when you want to plot multiple variables on the same plot, e.g., a line plot of each of the three variables (pop, lifeExp, and gdpPercap) over time.
